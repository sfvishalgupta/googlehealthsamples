...
class Zend_Gdata_Health_ProfileFeedTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testGetFirstEntrysCCRMedication()
    {
        ...
        $meds = $this->profileFeed->entry[0]->getCCR()->getMedications();
        $this->assertType('array', $meds);
        $this->assertEquals(1, count($meds));
        
        $this->assertXmlStringEqualsXmlString(file_get_contents(
                'Zend/Gdata/Health/_files/TestDataHealthProfileEntrySample_medications0.xml', true), 
                $meds[0]->getXML());
